(() => {
  ...
  if (backLink && (sourceContext === 'asset' || sourceContext === 'search')) {
    backLink.textContent = sourceContext === 'asset' ? 'Back to asset profile' : 'Back to search';
    backLink.href = '#';
  ...
  function text(row) { return String(row?.metric_label || '').toLocaleLowerCase('en'); }
  ...
  function isRatio(row) { return row?.raw_unit === 'RATIO' || row?.unit_label === 'x' || /ev\/ebitda|p\/e|p\/b|p\/s|debt\/ebitda/.test(text(row)); }
  function isPercent(row) { return row?.raw_unit === 'PERCENT' || row?.unit_label === '%' || /margin|profitability|(^|\W)roe($|\W)|(^|\W)roa($|\W)/.test(text(row)); }
  function isArtifact(row) { return Number(row?.value)===0 && /revenue|ebitda|profit|asset|equity|debt|cash|capex/.test(text(row)); }
  ...
  function role(row) {
    const t=text(row);
    if(t.includes('revenue') && !t.includes('/')) return 'revenue';
    if(t.includes('ebitda') && !t.includes('/') && !t.includes('ev/') && !t.includes('rentab')) return 'ebitda';
    if(t.includes('operating profit') && !t.includes('/')) return 'operating_profit';
    if(t.includes('net profit') && !t.includes('/')) return 'profit';
    if(t.includes('total operating income')) return 'operating_income';
    if(t.includes('net interest income')) return 'net_interest_income';
    if(t.includes('net fee and commission income')) return 'net_fee_income';
    if(t.includes('operating cash flow')) return 'ocf';
    if(t.includes('free cash flow')) return 'fcf';
    if(t.includes('capex') && !t.includes('/')) return 'capex';
    if(t.includes('operating margin')) return 'operating_margin';
    if(t.includes('ebitda margin')) return 'ebitda_margin';
    if(t.includes('income profitability')) return 'bank_profitability';
    if(t.includes('net profit margin')) return 'net_margin';
    if(t.includes('net interest margin')) return 'nim';
    if(t.includes('cost-to-income') || t.includes('(cir)')) return 'cir';
    if(/(^|\W)roa($|\W)/.test(t)) return 'roa';
    if(/(^|\W)roe($|\W)/.test(t)) return 'roe';
    if(t.includes('net debt/ebitda')) return 'net_debt_ebitda';
  ...
  function profitabilitySeries(key) {
    ...
    const formula={operating_margin:['operating_profit','revenue','Operating margin'],ebitda_margin:['ebitda','revenue','EBITDA margin'],net_margin:['profit','revenue','Net profit margin'],bank_profitability:['profit','operating_income','Income profitability']}[key];
    return formula?derivedPercentSeries(key,...formula):[];
  }
  function cleanUnit(row) { return String(row?.unit_label||'').replace(/\bRUB\b|RUB\.?/ig,'').trim(); }
  ...
  function empty(node, message='Insufficient comparable data.') { node.innerHTML=`<div class="empty">${esc(message)}</div>`; }
  ...
  function resultDefinitions() {
    if(isBankingSlice()) return [['operating_income','Total operating income'],['net_interest_income','Net interest income'],['profit','Net profit']];
    if(!hasRole('ebitda')&&hasRole('operating_profit')) return [['revenue','Revenue'],['operating_profit','Operating profit'],['profit','Net profit']];
    return [['revenue','Revenue'],['ebitda','EBITDA'],['profit','Net profit']];
  }
  function profitabilityGroups() {
    return isBankingSlice()
      ? {margins:{label:'Profitability and efficiency',defs:[['bank_profitability','Income profitability'],['nim','Net interest margin'],['cir','Cost-to-income ratio']]},returns:{label:'Returns on capital',defs:[['roa','ROA'],['roe','ROE']]}}
      : {margins:{label:'Profit margins',defs:[['operating_margin','Operating margin'],['ebitda_margin','EBITDA margin'],['net_margin','Net profit margin']]},returns:{label:'Returns on capital',defs:[['roa','ROA'],['roe','ROE']]}};
  }
  ...
  function renderControls() {
    const standards=unique(rows.map(r=>r.standard)); segment('standardButtons',standards,{IFRS:'IFRS',RAS:'RAS',UNKNOWN:'Other'},state.standard,v=>{state.standard=v; const p=unique(rows.filter(r=>r.standard===v).map(r=>r.period_type)); state.periodType=p.includes('annual')?'annual':p[0]||''; state.metric=defaultMetricCode(); render();});
    const periods=unique(rows.filter(r=>r.standard===state.standard).map(r=>r.period_type)); segment('periodButtons',periods,{annual:'Annual',quarterly:'Quarterly',unknown:'Periods'},state.periodType,v=>{state.periodType=v;state.metric=defaultMetricCode();render();});
  ...
  function renderMetrics() {
    ...
    const supplemental=[['net_debt_ebitda','Net debt / EBITDA'],['ocf','Operating cash flow'],['fcf','Free cash flow'],['roe','ROE'],['ebitda_margin','EBITDA margin'],['net_margin','Net profit margin'],['capex','CAPEX'],['ev_ebitda','EV / EBITDA'],['pe','P / E']];
    ...
    const cards=defs.map(([key,title])=>{const s=seriesByRole(key), row=latest(s.filter(r=>r.period!=='LTM'))||latest(s); if(!row)return ''; const delta=isPercent(row)||isRatio(row)?ppChange(s):yoy(s); const cls=delta===null?'flat':delta>=0?'up':'down'; const suffix=isPercent(row)?' pp':isRatio(row)?'x':'%'; const basis=isPercent(row)||isRatio(row)?'change':'YoY'; return `<article class="metric-card"><span>${title}</span><strong>${esc(display(row))}</strong><div><b class="delta ${cls}">${delta===null?'—':`${delta>=0?'+':''}${nf.format(delta)}${suffix}`}</b><small>${esc(row.period)} · ${basis}</small></div></article>`;}).filter(Boolean);
    const fallbackCards=[
      ['Available metrics',new Set(filtered().map(row=>row.metric_code).filter(Boolean)).size,'metrics'],
      ['Reporting periods',new Set(filtered().map(row=>row.period).filter(period=>period&&period!=='LTM')).size,'periods'],
      ['Observations',valid(filtered()).length,'data points']
    ];
    fallbackCards.forEach(([title,value,note])=>{if(cards.length<4)cards.push(`<article class="metric-card"><span>${title}</span><strong>${nf.format(value)}</strong><div><b class="delta flat">${note}</b><small>in the selected view</small></div></article>`)});
    document.getElementById('latestMetrics').innerHTML=cards.length?cards.join(''):'<div class="empty">No key metrics available.</div>';
  }
  function renderResults() {
    const defs=resultDefinitions().map(([key,name])=>[key,name,semanticColor(key)]);
    document.getElementById('resultsTitle').textContent=defs.map(([,name])=>name).join(', ').replace(/, ([^,]+)$/,' and $1');
    ...
    document.getElementById('resultsPeriod').textContent=`${state.standard==='IFRS'?'IFRS':state.standard} · ${state.periodType==='annual'?'annual':'quarterly'}`;
  }
  function renderMargins() {
    ...
    document.getElementById('marginFacts').innerHTML=sets.map(x=>{const r=latest(x.data),d=ppChange(x.data);return `<div><span>${esc(x.name)}</span><strong>${esc(display(r))}</strong><small>${r?.derived?'Calculated metric':d===null?'Change n/a':`${d>=0?'+':''}${nf.format(d)} pp`}</small></div>`}).join('');
    const bank=isBankingSlice(),returns=state.profitabilityView==='returns';
    document.getElementById('marginMethodology').textContent=returns?'ROA and ROE measure returns on average assets and average equity based on reported financial statements.':bank?'Income profitability is net income divided by total operating income. NIM and CIR are used as reported by the bank; a lower CIR indicates improved efficiency.':'Operating, EBITDA and net margins are calculated relative to revenue; issuer-reported figures take precedence.';
  }
  const peerLabels={revenue:'Revenue',operating_income:'Total operating income',net_interest_income:'Net interest income',net_fee_income:'Net fee and commission income',ebitda:'EBITDA',operating_profit:'Operating profit',profit:'Net profit',ebitda_margin:'EBITDA margin',nim:'Net interest margin',cir:'Cost-to-income ratio',net_margin:'Net profit margin',roe:'ROE',net_debt_ebitda:'Net debt / EBITDA',ev_ebitda:'EV / EBITDA',pe:'P / E'};
  function renderPeers() {
    ...
    if(!available.includes(state.peerMetric)) state.peerMetric=isBankingSlice()&&available.includes('operating_income')?'operating_income':available.includes('revenue')?'revenue':available.includes('operating_income')?'operating_income':available.includes('ebitda_margin')?'ebitda_margin':available[0];
    ...
    peerTable.innerHTML=`<table class="peer-table"><thead><tr><th>Issuer</th><th>Metric</th><th>Versus median</th></tr></thead><tbody>${items.map(i=>{const href=`company.html?ticker=${encodeURIComponent(i.ticker)}&from=peer`;return `<tr class="peer-table__row ${i.ticker===current?'current':''}" data-href="${esc(href)}" tabindex="0" role="link" aria-label="Open financial statements: ${esc(i.company_name||i.ticker)}"><td><a class="peer-table__link" href="${esc(href)}"><b>${esc(i.ticker)}</b><span>${esc(i.company_name)}</span></a></td><td>${esc(displayPeer(i))}</td><td>${median?`${i.value>=median?'+':''}${nf.format((i.value/median-1)*100)}%`:'—'}</td></tr>`;}).join('')}</tbody></table>`;
  ...
  function renderStatement() {
    ...
    bindChartTooltip('focusChart',point=>({period:periodLabel(point.x),label:document.getElementById('metricSelect').selectedOptions[0]?.textContent||'Metric',value:point.customdata,color:barColors[point.pointNumber]||accent}));
    document.getElementById('companyTableWrap').innerHTML=`<table class="company-table"><thead><tr><th>Period</th><th>Value</th><th>Change</th></tr></thead><tbody>${points.slice().reverse().map((r,i,a)=>{const previous=a[i+1];const d=previous&&Number(previous.value)?(Number(r.value)/Number(previous.value)-1)*100:null;return `<tr><td>${esc(periodLabel(r.period))}</td><td class="value">${esc(display(r))}</td><td class="${d!==null&&d<0?'negative':'positive'}">${d===null?'—':`${d>=0?'+':''}${nf.format(d)}%`}</td></tr>`}).join('')}</tbody></table>`;
  }
