(() => {
  ...
  const ratio = new Intl.NumberFormat('en-GB', { maximumFractionDigits: 2 });
  ...
  function normalizeSearch(value) {
    return String(value ?? '')
      ...
      .replace(/e/g, 'e')
      .replace(/[^a-za-ya0-9]+/giu, ' ')
      .trim()
      .replace(/\s+/g, ' ');
  }

  function standardLabel(value) {
    return value === 'IFRS' ? 'IFRS' : value === 'RAS' ? 'RAS' : value === 'UNKNOWN' ? 'Other' : value;
  }
  ...
  function issuerWord(value) {
    ...
    if (mod100 >= 11 && mod100 <= 14) return 'issuers';
    if (mod10 === 1) return 'issuer';
    if (mod10 >= 2 && mod10 <= 4) return 'issuer';
    return 'issuers';
  }
  ...
  function filtered() {
    ...
    return companies.filter(item => {
      ...
      return (!query || tokens.every(token => haystack.includes(token)))
        && (!state.sectors.size || state.sectors.has(item.sector || 'Sector not specified'))
  ...
  function sortValue(item, key) {
    ...
    if (key === 'sector') return item.sector || 'Sector not specified';
  ...
  function setupSorting() {
    table.querySelectorAll('th[data-sort]').forEach(th => {
      ...
      th.innerHTML = `<button class="statements-sort-button" type="button" data-sort-key="${esc(key)}" aria-label="Sort by: ${esc(accessibleLabel)}"><span class="statements-sort-label">${label}</span><img class="statements-sort-icon" src="../assets/icons/chevron-down.png" alt=""></button>`;
  ...
  function companyCell(item) {
    return `<div class="company-cell">
      ...
      <div class="company-copy"><strong>${esc(item.company_name)}</strong><span>${esc(item.display_code || item.ticker)} · ${esc(item.isin || 'ISIN not available')}</span></div>
    </div>`;
  }
  ...
  function rowHtml(item) {
    const snapshot = selectedSnapshot(item);
    const action = `<td class="statement-action"><a class="statement-link" href="${esc(item.page)}">Open</a></td>`;
    const leading = `<td>${companyCell(item)}</td><td>${esc(item.sector || 'Sector not specified')}</td>${action}<td>${snapshot.standard ? `<span class="coverage-tag">${esc(standardLabel(snapshot.standard))}</span>` : '—'}</td><td>${snapshot.values ? esc(periodLabel(state.period)) : '—'}</td>`;
  ...
  function render() {
    ...
    table.querySelector('tbody').innerHTML = visible.length
      ...
      : '<tr><td colspan="19"><div class="empty-state">No issuers match the selected criteria.</div></td></tr>';
    document.getElementById('statementsMeta').innerHTML = `Showing <strong>${visible.length}</strong> of <strong>${rows.length}</strong> · ${esc(periodLabel(state.period))}`;
    document.getElementById('statementsSummary').textContent = `${rows.length} ${issuerWord(rows.length)}`;
    document.getElementById('statementsPageLabel').textContent = `Page ${state.page} of ${pages}`;
