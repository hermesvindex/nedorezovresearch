(function () {
  ...
  const title = data.company_name || data.ticker || "Issuer";
  document.title = title + " — financial statements";
  ...
  if (chips) {
    ...
    if (Array.isArray(data.records)) values.push(data.records.length.toLocaleString("en-GB") + " observations");
  ...
  if (peerTitle) peerTitle.textContent = (data.sector ? data.sector + ": " : "") + "issuer and comparable companies";
}());
