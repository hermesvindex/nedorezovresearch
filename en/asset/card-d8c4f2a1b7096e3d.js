        ...
        if (asset.assetType === "macro") {
          ...
          const macroSummary = {
            annual: {
              ...
              unit: "% year to date",
              label: `Yearovaya inflyatsiya · on ${dateRu(asset.asof)}`,
              delta: "nakoplennym itogom · partial year",
            },
            monthly: {
            ...
            monthlyAnnual: {
              value: asset.price,
              unit: "% year on year",
              label: `Monthly inflation at an annualised rate · ${asset.periodLabel || dateRu(asset.asof)}`,
          ...
          const metrics = [
            ["One month", `${num(asset.monthlyChange, 2)}%`, "izmenenie potrebitelskikh tsen", false],
            ["Year to date", `${num(asset.ytdChange, 2)}%`, "nakoplennym itogom", false],
            ["Year on year", `${num(asset.price, 2)}%`, asset.periodLabel || "", false],
          ];
        ...
        document.getElementById("cashflow").innerHTML = rows.map(item => `<div class="flow-row">
          ...
          <div class="flow-main">
            ...
            <div class="flow-note">${item.complete
              ...
              : `partial year · latest month ${num(item.monthly, 2)}% · year on year ${num(item.price, 2)}%`}</div>
          </div>
...
: `${num(row.price, 2)}${asset.assetType === "bond" ? "%" : ` ${currencyLabel(asset.currency)}`}`;
    ...
    function renderChart(asset) {
      ...
      const macroSeries = {
        annual: { key: "ytd", label: "Yearovaya inflyatsiya year to date, %" },
        monthly: { key: "monthly", label: "Monthly inflation, %" },
        monthlyAnnual: { key: "price", label: "Monthly inflation at an annualised rate, %" },
      };
