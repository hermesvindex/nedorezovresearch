...
document.title = `${ASSETS[0].title || ASSETS[0].secid || "Aktiv"} · Asset profile`;
const state = {
  assetIndex: 0,
  ...
  mode: ASSETS[0].assetType === "macro" ? "monthlyAnnual" : "price",
    ...
    const rub = new Intl.NumberFormat("en-GB", { maximumFractionDigits: 0 });
    const rubFine = new Intl.NumberFormat("en-GB", { minimumFractionDigits: 0, maximumFractionDigits: 2 });
    const rubTiny = new Intl.NumberFormat("en-GB", { minimumFractionDigits: 0, maximumFractionDigits: 6 });
    ...
    const dividendNote = item => item.yieldPct === null || item.yieldPct === undefined || Number.isNaN(item.yieldPct) ? "percentage unavailable" : "payment history";
    const compactMoney = (value, currency = "RUB") => {
      if (value === null || value === undefined || Number.isNaN(value)) return "—";
      const unit = currencyLabel(currency);
      const abs = Math.abs(value);
      if (abs >= 1_000_000_000_000) return `${num(value / 1_000_000_000_000, 2)}\u00a0tn\u00a0${unit}`;
      if (abs >= 1_000_000_000) return `${num(value / 1_000_000_000, 2)}\u00a0bn\u00a0${unit}`;
      if (abs >= 1_000_000) return `${num(value / 1_000_000, 1)}\u00a0mn\u00a0${unit}`;
      return money(value, currency);
    };
    const couponStatus = value => value === "estimate" || value === "forecast" ? "forecast" : value === "known" ? "per schedule" : value === "schedule" ? "per schedule" : "actual";
    ...
    const signedPoints = value => value === null || value === undefined || Number.isNaN(value)
      ...
      : Math.abs(value) < .005
        ? "0 pp"
        : `${value > 0 ? "+" : "−"}${num(Math.abs(value), 2)} pp`;
    const periodLabel = value => ({ D: "Day", W: "Week", M: "Month", "3M": "3M", "6M": "6M", YTD: "YTD", "1Y": "Year", "3Y": "3Y", "5Y": "5Y", ALL: "All" }[value] || value);
    ...
    function tileValueHtml(value) {
      ...
      const moneyLike = /[₽$€¥]|mn|bn|thousand/i.test(raw);
    ...
    function renderPeriodReturns(asset) {
      ...
      wrap.innerHTML = periods.map(period => {
        ...
        const note = period === "ALL" && asset.firstObservation ? `since ${dateRu(asset.firstObservation)}` : "";
    ...
    function renderSummary(asset) {
      ...
      if (asset.assetType && asset.assetType !== "bond") {
        if (asset.assetType === "rate") {
          ...
          document.getElementById("chips").innerHTML = [
            asset.publisher || "Bank of Russia",
            asset.quoteUnit || "% p.a.",
          ].map(item => `<span class="chip tint">${item}</span>`).join("");
          document.querySelector('[data-mode="price"]').textContent = "Rate";
          document.getElementById("price").textContent = `${num(asset.price, asset.valueDigits ?? 2)} %`;
          document.getElementById("dirtyPrice").textContent = "Bank of Russia key rate";
          setDelta(
            asset.deltaPoints,
            `${signedPoints(asset.deltaPoints)} · reshenie ${dateShort(asset.lastDecisionDate)}`
          );
          document.getElementById("metrics").innerHTML = "";
          document.getElementById("tiles").innerHTML = "";
          document.getElementById("description").innerHTML = "";
          document.getElementById("footerNote").textContent =
            `Historical series — Bank of Russia · Quantis forecast shown separately from the chart · data as of ${dateRu(asset.asof)}`;
          return;
        }
        if (asset.assetType === "macro") {
          ...
          const macroSummary = {
            annual: {
              ...
              unit: "% year to date",
              label: `Nakoplennaya inflyatsiya · ${asset.periodLabel || dateRu(asset.asof)}`,
              delta: "izmenenie tsen year to date",
            },
            monthly: {
              value: asset.monthlyChange,
              unit: "% for mesyats",
              label: `Monthly inflation · ${asset.periodLabel || dateRu(asset.asof)}`,
              delta: "izmenenie potrebitelskikh tsen for mesyats",
            },
            monthlyAnnual: {
              value: asset.price,
              unit: "% for 12 mesyatsev",
              label: `Yearovaya inflyatsiya · ${asset.periodLabel || dateRu(asset.asof)}`,
              delta: `${signedPoints(asset.deltaPoints)} to predydushchemu mesyatsu`,
            },
          }[state.mode] || {
            value: asset.price,
            unit: "% for 12 mesyatsev",
          ...
          const metrics = [
            ["Monthly inflation", `${num(asset.monthlyChange, 2)}%`, asset.periodLabel || "for mesyats", false],
            ["Nakoplennaya inflyatsiya", `${num(asset.ytdChange, 2)}%`, "year to date", false],
            ["Yearovaya inflyatsiya", `${num(asset.price, 2)}%`, "for poslednie 12 mesyatsev", false],
          ];
          ...
          document.getElementById("tiles").innerHTML = renderTiles(recentYears.map(row => [
            ...
            row.complete ? `${row.year} year` : `${row.year} · on ${dateRu(row.date)}`,
          ]));
          ...
          const source = sourceRows.length
            ...
            : asset.sourceUrl
              ? `<a href="${htmlEsc(asset.sourceUrl)}" target="_blank" rel="noopener noreferrer">${htmlEsc(asset.sourceLabel || "Source")}</a>`
              : htmlEsc(asset.sourceLabel || "Source table");
          document.getElementById("description").innerHTML =
            `${htmlEsc(asset.description || "")}<div class="formula"><strong>${htmlEsc(asset.methodology || "")}</strong><span>${htmlEsc(asset.marketNote || "")}</span><span>Source: ${source}</span></div>`;
          document.getElementById("footerNote").textContent =
            `Mesyachnyy ryad · data for ${asset.periodLabel || dateRu(asset.asof)}`;
          return;
        }
        ...
        document.getElementById("chips").innerHTML = [
          ...
          asset.secid,
          asset.board,
          isMarketAsset ? asset.class : (isFund ? "BPIF" : "Equity"),
        ...
        document.getElementById("dirtyPrice").textContent = isMarketAsset ? (asset.valueLabel || "Latest value") : (isFund ? "Estimated unit value" : "Closing price");
        ...
        setDelta(dayDelta, dayDelta === null || dayDelta === undefined ? "—" : isMarketAsset
          ? `${absoluteSign}${marketValue(Math.abs(dayDelta))} on the day (${signedPct(dayDeltaPct)})`
          : `${absoluteSign}${money(Math.abs(dayDelta), asset.currency, 2)} on the day (${signedPct(dayDeltaPct)})`);
        const metrics = isMarketAsset ? [
          ["Day", signedPct(asset.priceDeltaDayPct), "change in latest value", false],
          ["Risk", pct(asset.annualVol), "annualised volatility", false],
          ["Asset class", asset.marketClass || asset.class || "—", "", false],
        ] : isFund ? [
          ["Annualised return", pct(asset.annualReturn), asset.note || "from the portfolio model", false],
          ["Risk", pct(asset.annualVol), "annualised volatility", false],
          [asset.rating ? "Credit rating" : "Asset Manager", asset.rating || asset.etfCompany || "—", asset.rating ? "MOEX" : "", false],
          ["Composition", asset.etfStructure || "—", "", false],
        ] : [
          ["Day", signedPct(asset.priceDeltaDayPct), "change in closing price", false],
          ["Risk", pct(asset.annualVol), "annualised volatility", false],
          ["Turnover", asset.turnover ? compactMoney(asset.turnover) : "—", "latest trading day", false],
          [asset.rating ? "Credit rating" : "Sector", asset.rating || asset.sector || "—", asset.rating ? "MOEX" : "", false],
        ];
        document.getElementById("metrics").innerHTML = metrics.map(([label, value, note, hot]) => {
          return `<div class="metric ${hot ? "hot" : ""}"><div class="${metricValueClass(value)}">${value}</div><div class="metric-label">${label}</div>${note ? `<div class="metric-note">${note}</div>` : ""}</div>`;
        }).join("");
        ...
        document.getElementById("tiles").innerHTML = renderTiles(isMarketAsset ? [
          [signedPct(asset.returns?.W), "Week"],
          [signedPct(asset.returns?.M), "Month"],
          [signedPct(asset.returns?.YTD), "Year to date"],
          [signedPct(asset.returns?.["1Y"]), "One year"],
          [dateRu(asset.firstObservation), "Series start"],
          [num(asset.observationCount, 0), "Observations"],
        ] : isFund ? [
          [asset.etfCompany || "—", "Asset manager"],
          [asset.etfStructure || "—", "Composition"],
          [asset.avgTurnover ? compactMoney(asset.avgTurnover) : "—", "Average turnover over 60 trading days"],
          [pct(asset.annualReturn), "Expected return"],
          [pct(asset.annualVol), "Risk"],
          [asset.isin || "—", "ISIN"],
        ] : [
          [asset.marketCap ? compactMoney(asset.marketCap) : "—", "Market capitalisation"],
          [asset.turnover ? compactMoney(asset.turnover) : "—", "Daily turnover"],
          [asset.volume ? nb(rub.format(asset.volume)) : "—", "Volume, units"],
          [asset.open ? money(asset.open, asset.currency, 2) : "—", "Opening price"],
        ...
        const extra = isMarketAsset
          ? `<div class="formula"><strong>${asset.methodology || asset.class || "Market indicator"}</strong><span>${asset.marketNote || "Historical performance is calculated from the local time series."}</span></div>`
          : isFund
          ? `<div class="formula"><strong>${asset.etfStructure || "ETF holdings"}</strong><span>Asset Manager: ${asset.etfCompany || "—"}</span>${asset.note ? `<span>${asset.note}</span>` : ""}</div>`
          : `<div class="formula"><strong>${asset.sector || "Sector"}</strong><span>${[asset.industry, asset.issuer].filter(Boolean).join(" · ") || "Sector data from the local reference dataset"}</span></div>`;
        document.getElementById("description").innerHTML = `${asset.description || "Description unavailable."}${extra}`;
        document.getElementById("footerNote").textContent = isMarketAsset
          ? `Data as of ${dateRu(asset.asof)}`
          : `Source: local moex_library/packages/${asset.secid} · portfolio_interactive · data on ${dateRu(asset.asof)}`;
        return;
      }
      renderPeriodReturns(asset);
      ...
      document.getElementById("dirtyPrice").textContent = asset.dirtyPrice !== null && asset.dirtyPrice !== undefined
        ...
        : asset.price !== null && asset.price !== undefined
          ...
          : "MOEX quote unavailable";
      const dayDelta = asset.priceDeltaDay;
      const dayDeltaPct = asset.priceDeltaDayPct;
      const sign = (dayDelta || 0) >= 0 ? "+" : "";
      setDelta(dayDelta, dayDelta === null || dayDelta === undefined ? "—" : `${sign}${num(dayDelta)} pp on the day (${sign}${pct(dayDeltaPct)})`);
      const floatingRateReference = asset.couponBenchmark === "Bank of Russia key rate" && asset.keyRate
        ? ` · KS TsB ${pct(asset.keyRate.value)}`
        : "";
      ...
      const metrics = [];
      if (asset.couponType === "Structured") {
        metrics.push(["Type", "Structured", "coupon and yield depend on the product terms", false]);
        metrics.push(["Maturity", dateRu(asset.maturityDate), "", false]);
        metrics.push(["Accrued interest", money(asset.accint), "", false]);
        metrics.push(["Credit rating", asset.rating || "—", "", false]);
      } else if (asset.couponType === "Floating rate") {
        metrics.push(["Coupon rate", pct(asset.couponRate), couponNote || floatingYieldNote, false]);
        metrics.push(["Current yield", pct(asset.currentYield), floatingYieldNote || "annual coupon divided by clean price", false]);
        metrics.push(["Accrued interest", money(asset.accint), "", false]);
        metrics.push(["Credit rating", asset.rating || "—", "", false]);
      } else {
        metrics.push(["Coupon", pct(asset.couponRate), couponNote, false]);
        metrics.push(["Current yield", pct(asset.currentYield), "annual coupon divided by clean price", false]);
        metrics.push(["Yield to maturity / MOEX", pct(asset.ytm), "", false]);
        metrics.push(["Credit rating", asset.rating || "—", "", false]);
      }
      document.getElementById("metrics").innerHTML = metrics.map(([label, value, note, hot]) => {
        return `<div class="metric ${hot ? "hot" : ""}"><div class="${metricValueClass(value)}">${value}</div><div class="metric-label">${label}</div>${note ? `<div class="metric-note">${note}</div>` : ""}</div>`;
      ...
      document.getElementById("tiles").innerHTML = renderTiles([
        [money(asset.faceValue, asset.currency), "Face value"],
        [asset.avgTurnover ? compactMoney(asset.avgTurnover) : "—", "Average turnover over 60 trading days"],
        [asset.frequency ? num(asset.frequency, 0) : "—", "Coupons per year"],
        [dateRu(asset.issueDate), "First trading date"],
        [dateRu(asset.nextCouponDate), "Next coupon"],
        [dateRu(asset.maturityDate), `Maturity${asset.yearsToMaturity ? ` · ${num(asset.yearsToMaturity, 1)} years` : ""}`]
      ]);
      const formulaReference = asset.couponBenchmark === "Bank of Russia key rate" && asset.keyRate
        ? `<span>Reference rate: Bank of Russia key rate ${pct(asset.keyRate.value)} on ${dateRu(asset.keyRate.date)}.</span>`
        : asset.couponBenchmark
          ? `<span>Reference rate: ${asset.couponBenchmark}.</span>`
          : "";
      ...
      document.getElementById("footerNote").textContent = `Source: local moex_library/packages/${asset.secid} · data on ${dateRu(asset.asof)}`;
    }
    ...
    function renderDetailCta(asset) {
      ...
      wrap.innerHTML = `<a class="detail-link" href="${htmlEsc(url)}" target="_top">Financial statements</a>`;
    }

    function renderCashflow(asset) {
      if (asset.assetType === "macro") {
        document.getElementById("cashflowTitle").textContent = "Annual inflation";
        ...
        document.getElementById("cashflow").innerHTML = rows.map(item => `<div class="flow-row">
          ...
          <div class="flow-main">
            <div class="flow-title">${item.complete
              ? `In ${item.year} godu inflation was ${num(item.annualValue, 2)}%`
              : `In ${item.year} godu to ${dateRu(item.date)} inflation was ${num(item.annualValue, 2)}%`}</div>
            <div class="flow-note">${item.complete
              ? "itog for yanvar–dekabr"
              : `partial year · latest month ${num(item.monthly, 2)}% · annual inflation ${num(item.price, 2)}%`}</div>
          </div>
      ...
      if (asset.assetType === "rate") {
        document.getElementById("cashflowTitle").textContent = "Decisions and forecast";
        ...
        document.getElementById("cashflow").innerHTML = rows.map(item => {
          ...
          const note = isForecast
            ? item.label || "Quantis forecast"
            : `Actual · effective from ${dateShort(item.effectiveDate)}`;
          return `<div class="flow-row ${isForecast ? "rate-forecast" : "past"}">
            <div class="flow-date">${dateShort(item.date)}</div>
            <div class="flow-main">
              <div class="flow-title">${isForecast ? "Meeting" : "Latest decision"}</div>
      ...
      if (["currency", "index", "commodity", "rate"].includes(asset.assetType)) {
        document.getElementById("cashflowTitle").textContent = "Security details";
      ...
      if (asset.assetType === "stock") {
        document.getElementById("cashflowTitle").textContent = "Dividends";
        ...
        document.getElementById("cashflow").innerHTML = rows.length ? rows.map(item => `<div class="flow-row">
          ...
          <div class="flow-main"><div class="flow-title">Dividend</div><div class="flow-note">${dividendNote(item)}</div></div>
          <div class="flow-value">${dividendValue(item, asset)}</div>
        </div>`).join("") : `<div class="desc">The local dataset does not include a dividend calendar.</div>`;
        return;
      }
      if (asset.assetType === "fund") {
        document.getElementById("cashflowTitle").textContent = "ETF holdings";
      ...
      document.getElementById("cashflowTitle").textContent = "Cash flow";
      const couponRows = (asset.futureCoupons || []).map(item => ({ ...item, type: "Coupon" })).sort((a, b) => String(a.date).localeCompare(String(b.date)));
      const yearEnd = `${new Date().getFullYear()}-12-31`;
      const coupons = asset.couponType === "Floating rate" ? couponRows.filter(item => String(item.date) <= yearEnd) : couponRows.slice(0, 6);
      const amort = (asset.futureAmortizations || []).map(item => ({ ...item, type: "Maturity" })).sort((a, b) => String(a.date).localeCompare(String(b.date))).slice(0, 2);
      if (!amort.length && asset.maturityDate) amort.push({ date: asset.maturityDate, value: asset.faceValue, rate: 100, currency: asset.currency, type: "Maturity" });
      ...
      const missingCouponHtml = !couponRows.length
        ? `<div class="desc">Future coupons are not disclosed in the local dataset${asset.couponType === "Floating rate" ? ": the rate is usually set closer to the coupon period." : "."}</div>`
        : "";
      const cashflowHtml = rows.length ? rows.map(item => {
        return `<div class="flow-row">
          <div class="flow-date">${dateRu(item.date)}</div>
          <div class="flow-main"><div class="flow-title">${item.type}${item.type === "Coupon" && item.rate ? ` · ${pct(item.rate)}` : ""}</div><div class="flow-note">${item.type === "Maturity" ? "principal · scheduled payment" : item.status ? couponStatus(item.status) : "scheduled payment"}</div></div>
          <div class="flow-value">${money(item.value, item.currency || asset.currency, item.type === "Coupon" ? 2 : 0)}</div>
        </div>`;
      }).join("") : `<div class="desc">Future coupons are not disclosed in the local dataset. For floating-rate bonds, the rate is commonly set closer to the coupon period.</div>`;
      const historyHtml = historyRows.length ? `<details class="coupon-history"><summary><span>Historical coupons</span><span>${historyRows.length} payments</span></summary><div class="coupon-table">${historyRows.map(item => {
        return `<div class="flow-row past">
          <div class="flow-date">${dateRu(item.date)}</div>
          <div class="flow-main"><div class="flow-title">Coupon${item.rate ? ` · ${pct(item.rate)}` : ""}</div><div class="flow-note">actual</div></div>
...
: `${num(row.price, 2)}${asset.assetType === "bond" ? "%" : ` ${currencyLabel(asset.currency)}`}`;
        const turnoverMetric = ["rate", "macro"].includes(asset.assetType) || !Number(row.turnover)
          ? ""
          : `<span class="asset-chart-tooltip__metric"><i style="--tooltip-color:rgba(16,18,21,.20)"></i>Turnover</span><strong>${htmlEsc(money(row.turnover || 0, "RUB", 0))}</strong>`;
    ...
    function renderChart(asset) {
      ...
      const macroSeries = {
        annual: { key: "ytd", label: "Nakoplennaya inflyatsiya, %" },
        monthly: { key: "monthly", label: "Monthly inflation, %" },
        monthlyAnnual: { key: "price", label: "Yearovaya inflyatsiya, %" },
      };
      ...
      const yTitle = selectedMacroSeries?.label
        || (state.mode === "yield" ? "Return, %" : (asset.chartLabel || (asset.assetType && asset.assetType !== "bond" ? `Price, ${asset.currency || "RUB"}` : "Price, %")));
      ...
      if (!hasPlotly) {
        document.getElementById("chart").innerHTML = "<div class='desc'>Plotly.js could not be loaded. Connect to the internet or place a local copy of plotly.min.js next to the HTML file.</div>";
        return;
      }
      if (!rows.length) {
        Plotly.purge("chart");
        document.getElementById("chart").innerHTML = `<div class="chart-empty">${state.mode === "yield" ? "MOEX yield history unavailable" : "MOEX price history unavailable"}</div>`;
        return;
      }
      const traces = [
        ...(showTurnover ? [{
