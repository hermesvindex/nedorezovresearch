(() => {
  ...
  const routes = [
    ['main', 'Home', '/'],
    ['heatmap', 'Equities', '/shares'],
    ['statements', 'Financials', '/reports'],
    ['bonds', 'Bonds', '/bonds'],
    ['portfolio', 'Portfolio builder', '/pm'],
    ['mature', 'Model portfolio', '/maturebonds'],
  ];
  ...
  searchButton.innerHTML = `<svg class="qn-control-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    ...
    </svg><span class="qn-control-label">Search</span>`;
  ...
  mobileMenuButton.innerHTML = `<svg class="qn-control-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    ...
    </svg><span class="qn-control-label">Menu</span>`;
  ...
  if (mainShell) {
    ...
    shell.setAttribute('aria-label', 'Navigation Nedorezov Research');
    shell.innerHTML = `
      <a class="quantis-global-shell__brand" href="${publicHref('/')}" target="_top">Nedorezov Research</a>
      <nav class="quantis-global-shell__nav" aria-label="Platform sections">
  ...
  mobile.setAttribute('aria-label', 'Osnovnye razdely');
  ...
  mobileSearch.textContent = 'Search';
    ...
    footer.innerHTML = `
      <div class="site-footer__top">
        <div class="site-footer__about">
          ...
          <p>An independent research platform for Russia's capital markets, covering equities, bonds, issuer financials, market indicators and portfolio analytics.</p>
        </div>
        <nav class="site-footer__nav" aria-label="Site sections">
          <p class="site-footer__label">Sections</p>
          ${routes.slice(1).map(([, label, href]) => `<a href="${publicHref(href)}" target="_top">${label}</a>`).join('')}
        </nav>
        <div class="site-footer__contacts">
          <p class="site-footer__label">Feedback</p>
          <p>Please contact the project owner if you identify an error in the data, calculations or interface.</p>
          <div class="site-footer__contact-actions">
            <a class="site-footer__contact" href="https://t.me/znedorezov" target="_top" rel="noopener noreferrer" aria-label="Message Zakhar Nedorezov on Telegram">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m22 2-7 20-4-9-9-4Z"></path><path d="M22 2 11 13"></path></svg>
              <span>Telegram</span>
            </a>
            <a class="site-footer__contact site-footer__contact--mail" href="mailto:nedorezov.zakhar@yahoo.com" aria-label="Email Zakhar Nedorezov">
      ...
      <div class="site-footer__risk">
        ...
        <p>The material on this website is provided for information and research purposes and does not constitute personal investment advice. The financial instruments and transactions discussed may be unsuitable for your financial circumstances, investment objectives, risk tolerance and return requirements.</p>
        <p>Investing involves the risk of partial or total loss of capital. Asset values can rise or fall; past performance and model forecasts do not determine future returns. Review the source data, assess the instrument terms and conduct your own risk analysis before making an investment decision.</p>
      </div>
  ...
  mobileMenuButton.addEventListener('click', () => {
    ...
    mobileMenuButton.setAttribute('aria-label', isOpen ? 'Close menyu' : 'Open menu');
  });
  ...
  assetModal.setAttribute('aria-label', 'Asset profile');
  assetModal.setAttribute('aria-hidden', 'true');
  assetModal.inert = true;
  assetModal.innerHTML = `<button class="quantis-asset-modal__backdrop" type="button" aria-label="Close card by clicking the backdrop"></button>
    <div class="quantis-asset-modal__panel">
      <button class="quantis-asset-modal__close" type="button" aria-label="Close card"><span aria-hidden="true"></span></button>
      <iframe class="quantis-asset-modal__frame" title="Asset profile"></iframe>
    </div>`;
  ...
  panel.setAttribute('aria-label', 'Asset search');
  panel.setAttribute('aria-hidden', 'true');
  panel.inert = true;
  panel.innerHTML = `
    <div class="quantis-search__head">
      <label class="quantis-search__field">
        <span class="sr-only">Asset, ticker or ISIN</span>
        <input id="quantisSearchInput" type="text" role="searchbox" inputmode="search" enterkeyhint="search" autocomplete="off" placeholder="Search" spellcheck="false">
        <kbd>ESC</kbd>
      </label>
      <button class="quantis-search__close" type="button">Close</button>
    </div>
    <div class="quantis-search__filters" role="tablist" aria-label="Asset class">
      <button type="button" data-kind="all" aria-selected="true">All</button>
      <button type="button" data-kind="stock" aria-selected="false">Equities</button>
      <button type="button" data-kind="bond" aria-selected="false">Bonds</button>
      <button type="button" data-kind="fund" aria-selected="false">Funds</button>
      <button type="button" data-kind="currency" aria-selected="false">Currencies</button>
      <button type="button" data-kind="index" aria-selected="false">Indices</button>
      <button type="button" data-kind="other" aria-selected="false">Other</button>
    </div>
  ...
  const excludedIndexTerms = ['alfa kapital', 'alfa-kapital', 'rayffayzen', 'raiffeisen', 'raif'];
  ...
  const formatMarketValue = item => {
    ...
    return `${item.valuePrefix || ''}${value.toLocaleString('en-GB',{maximumFractionDigits:digits})}${suffix}`;
  };
  const formatDayDelta = item => {
    if (item.kind === 'rate') {
      ...
      const absolute = Math.abs(points).toLocaleString('en-GB', { maximumFractionDigits: 2 });
      ...
      return `${sign}${absolute} pp${dateLabel ? ` · ${dateLabel}` : ''}`;
    }
    if (item.deltaPct === null || item.deltaPct === undefined || item.deltaPct === '') return '';
    const value = Number(item.deltaPct);
    if (!Number.isFinite(value)) return '';
    return `${value > 0 ? '+' : ''}${value.toLocaleString('en-GB',{maximumFractionDigits:2})}%`;
  };

  function loadIndex() {
    if (index.length) return Promise.resolve(index);
    if (loading) return loading;
    meta.textContent = 'Loading the instrument index…';
    ...
    loading = fetch(new URL('search-index.json?v=20260924-interface-quality-3', scriptUrl))
      ...
      .then(prepare)
      .catch(() => {
        meta.textContent = 'Index vremenno nedostupen. Obnovite stranitsu.';
        return [];
      });
    return loading;
  }
  ...
  function assetRow(item, position) {
    const detail = item.kind === 'bond' && item.maturity ? ` · maturity ${item.maturity}` : '';
  ...
  function actionRow(row, position) {
    ...
    const label = isBonds ? 'All bond issues' : 'Company profile and financial statements';
    const metaLabel = isBonds ? `${row.issuer.bondIds.length} issues · filter will be applied` : 'Issuer profile · financial statements';
    const href = isBonds
    ...
    return `<article class="quantis-search-result quantis-search-result--action" role="option" aria-selected="${position === selected}">
      <a class="quantis-search-result__main" href="${href}">
        <span class="quantis-search-result__ticker">${row.issuer.logo ? `<img src="${url(row.issuer.logo)}" alt="">` : (isBonds ? 'Issues' : 'Company')}</span>
        ...
        <span class="quantis-search-result__type">Open</span>
      </a>
    </article>`;
  }
  ...
  function render() {
    ...
    results.innerHTML = rows.length ? rows.map(resultRow).join('') : '<div class="quantis-search__empty">No results. Check the ticker, ISIN or instrument name.</div>';
    meta.textContent = needle
      ? (total > rows.length ? `Showing ${rows.length} naibolee relevantnykh rezultatov` : `Results: ${total}`)
      : '';
  }
...
(() => {
  ...
  const formatValue = point => {
    ...
    if (!Number.isFinite(value)) return '—';
    const name = String(point.data?.name || point.fullData?.name || 'Value');
    if (/turnover/i.test(name)) return `${value.toLocaleString('en-GB',{maximumFractionDigits:0})}\u00a0₽`;
    const suffix = /income|price, %/i.test(name) ? '%' : '';
    return `${value.toLocaleString('en-GB',{minimumFractionDigits:2,maximumFractionDigits:2})}${suffix}`;
  };
  const attach = chart => {
...
(() => {
  ...
  function enhance(select) {
    ...
    if (caption) caption.textContent = 'Rows';
...
(() => {
  ...
  function directionOf(button) {
    ...
    if (/prev|nazad|predydushch/.test(text)) return 'previous';
    if (/next|vper.d|dalee|sleduyushch/.test(text)) return 'next';
    return '';
  }
  ...
  function unifyPaginationLayout(root = document) {
    ...
    buttons.forEach(button => {
      ...
      if (directButtons.length >= 1) {
        ...
        parent.setAttribute('aria-label', 'Pagination');
      }
    });
  }
  ...
  function normalizeActionLabel(control) {
    ...
    if (control.matches('#addStockFilter, #addFilterMenu > summary')) {
      ...
      const isShow = /pokaz/i.test(current);
      visible = isShow ? 'Show' : 'Hide';
      label = isShow ? 'Show filters' : 'Hide filters';
    }
