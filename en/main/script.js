(() => {
  ...
  const percent = new Intl.NumberFormat('en-GB', { maximumFractionDigits: 2 });
  const dateFormat = new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
  const polymarketTimeFormat = new Intl.DateTimeFormat('en-GB', {
  ...
  function deltaText(item) {
    ...
    if (isMacro) {
      ...
      if (Math.abs(numeric) < .005) return '0 pp month on month';
      return `${numeric > 0 ? '+' : '−'}${percent.format(Math.abs(numeric))} pp month on month`;
    }
    if (isRate) {
      if (!Number.isFinite(numeric)) return '—';
      const date = String(item.lastDecisionDate || '').match(/^\d{4}-(\d{2})-(\d{2})$/);
      const dateLabel = date ? ` · ${date[2]}.${date[1]}` : '';
      if (Math.abs(numeric) < .005) return `0 pp${dateLabel}`;
      return `${numeric > 0 ? '+' : '−'}${percent.format(Math.abs(numeric))} pp${dateLabel}`;
    }
  ...
  const decisionPalette = {
    Decrease: { label: 'Rate cut', color: '#FF6A00' },
    'No Change': { label: 'No change', color: '#355E78' },
    Increase: { label: 'Rate increase', color: '#0F2233' }
  };
  const chartDateFormat = new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', timeZone: 'Europe/Moscow' });
  const tooltipDateFormat = new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Moscow' });
  ...
  function renderDecisionMarket(eventData, timestamp) {
    ...
    if (stamp) {
      const formatted = polymarketTimeFormat.format(timestamp).replace(',', ' in');
      stamp.textContent = `Polymarket · checked ${formatted} MSK`;
  ...
  function drawDecisionChart() {
    ...
    if (!decisionChartSeries.length) {
      ...
      context.fillText('Loading market history…', rect.width / 2, rect.height / 2);
      return;
    }
  ...
  function cardMarkup(item, copyIndex = carouselCenterCopy) {
    ...
    const logo = item.logo
      ...
      : '';
    return `<a class="market-card ${logo ? 'market-card--with-logo' : ''}" href="${escapeHtml(item.href)}" data-market-id="${escapeHtml(item.id)}" data-carousel-copy="${copyIndex}"${isClone ? ' aria-hidden="true" tabindex="-1"' : ` aria-label="Open asset profile: ${escapeHtml(item.name)}"`}>
  ...
  function renderCards() {
    ...
    Object.entries(tracks).forEach(([group, track]) => {
      ...
      track.innerHTML = ordered.length
        ? loopMarkup(ordered)
        : '<p class="market-card market-card--empty">Market data are temporarily unavailable.</p>';
