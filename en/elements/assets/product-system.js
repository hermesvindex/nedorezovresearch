...
(() => {
  ...
  function updateEnumSummary(details) {
    ...
    if (summary) summary.textContent = checked ? `Selected: ${checked}` : 'Select';
  }
  ...
  document.addEventListener('click', event => {
    ...
    card.innerHTML = `<div class="qn-filter-card__head"><p class="qn-filter-label">${add.textContent.trim()}</p><button class="qn-filter-remove" type="button" data-qn-remove aria-label="Remove filter">×</button></div><input class="qn-field" type="text" placeholder="">`;
    grid.append(card);
